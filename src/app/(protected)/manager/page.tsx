...
import { BarChart3, CalendarDays, CircleDollarSign, Package, Plus, RefreshCw, Wallet } from "lucide-react";
...
export default function ManagerPage() {
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900">
    <header className="flex flex-wrap items-center justify-between gap-3 border-b bg-white px-6 py-4">
      ...
      <div className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-slate-500" /><input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="rounded border px-3 py-2 text-sm" /><Button variant="secondary" size="sm" onClick={load}><RefreshCw className="h-4 w-4" /> Refresh</Button><Button size="sm" onClick={openUsers}><Plus className="h-4 w-4" /> Manage users</Button></div>
    </header>
