...
@frappe.whitelist(methods=["POST"])
def create_return(invoice_name, items=None):
	require_role("Swift Cashier")
	...
	if items:
		# Narrow down to only the requested items/qty if a partial return was given.
		requested = {row["item_code"]: flt(row["qty"]) for row in items}
		filtered_items = []
		for row in return_doc.items:
			if row.item_code in requested:
...
def get_invoice(invoice_name):
	...
	session = _get_open_session_for_user(frappe.session.user)
	is_owner = session and doc.custom_pos_opening_entry == session.name
	if not is_owner and doc.owner != frappe.session.user:
		frappe.local.response.http_status_code = 403
		frappe.throw(_("This invoice does not belong to your session."))
